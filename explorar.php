...
<div class="fade-in h-screen max-w-2xl mx-auto flex flex-col justify-center px-4 py-6">
    ...
    <div id="profile-card" class="relative w-full h-[70vh] rounded-[3.5rem] overflow-hidden shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] bg-slate-800 group transition-all">
        ...
        <div class="absolute bottom-10 left-0 right-0 flex justify-center items-center gap-3 lg:gap-4 z-30 px-4">
            ...
            <button onclick="parent.openPremium('Mandar Mensagem')" class="btn-action float-animation w-14 h-14 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-full flex items-center justify-center text-emerald-400 text-xl hover:bg-emerald-500 hover:text-white transition-all shadow-2xl" style="animation-delay: 0.2s;">
            ...
            <button onclick="parent.openPremium('Super Like')" class="btn-action float-animation w-14 h-14 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-full flex items-center justify-center text-blue-400 text-xl hover:bg-blue-400 hover:text-white transition-all shadow-2xl" style="animation-delay: 0.4s;">
...
<div id="modalFiltros" onclick="fecharFiltrosFora(event)">
    <div class="conteudo-filtros" onclick="event.stopPropagation()">
        ...
        <div class="campo-filtro">
            <label>Distância Máxima <span id="valDistancia">50km</span></label>
            <input type="range" id="distanciaFiltro" min="1" max="200" value="50" oninput="atualizarLabelsFiltros()">
        </div>
...
<div id="modal-perfil-explorar" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
    <div class="bg-[#1e293b] w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-700 max-h-[90vh] overflow-y-auto no-scrollbar relative">
        <div id="premium-overlay" class="absolute inset-0 z-[60] bg-slate-900/40 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center hidden">
            <div class="w-20 h-20 bg-amber-400 rounded-3xl flex items-center justify-center mb-6 shadow-2xl animate-bounce">
                <i class="fa-solid fa-crown text-white text-4xl"></i>
            </div>
            <h3 class="text-white text-2xl font-black mb-2 italic">Perfil Privado</h3>
            <p class="text-slate-300 text-sm mb-8 max-w-[250px]">Apenas membros <b>Connect Gold</b> podem ver detalhes e fotos dos momentos.</p>
            <button onclick="parent.openPremium('Ver Perfil Completo')" class="bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-900 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-all">
                DESBLOQUEAR AGORA
            </button>
        </div>
        ...
        <div id="dropdown-opcoes" class="hidden menu-opcoes">
            <button onclick="denunciarUsuario()" class="w-full text-left px-5 py-4 text-[10px] font-black uppercase tracking-widest text-slate-300 hover:bg-slate-800 transition border-b border-slate-700">
                <i class="fa-solid fa-circle-exclamation mr-2 text-amber-500"></i> Denunciar
            </button>
...
<script>
    ...
    function fecharFiltrosFora(e) {
        if(e.target.id === 'modalFiltros') fecharFiltros();
    }
    ...
    function atualizarLabelsFiltros() {
        ...
        document.getElementById('valDistancia').innerText = document.getElementById('distanciaFiltro').value + "km";
        document.getElementById('valFotos').innerText = document.getElementById('fotosMin').value;
    }

    function aplicarFiltros() {
        fecharFiltros(); 
        let todosOsPerfis = getProfilesFromDB(); 
        const minI = parseInt(document.getElementById('idadeMin').value);
        const maxI = parseInt(document.getElementById('idadeMax').value);
        const minF = parseInt(document.getElementById('fotosMin').value);

        const filtrados = todosOsPerfis.filter(p => {
            const partes = p.name.split(',');
            const idade = partes.length > 1 ? parseInt(partes[1].trim()) : 22;
            return idade >= minI && idade <= maxI && p.imgs.length >= minF;
        });
    ...
    function limparFiltros() {
        ...
        document.getElementById('distanciaFiltro').value = 50;
    ...
    function getProfilesFromDB() {
        let bancoGlobal = {};
        const usuarioLogadoNome = localStorage.getItem('usuario_logado') || "";

        try {
            if (window.parent && window.parent.usuariosDB) {
                bancoGlobal = window.parent.usuariosDB;
            } else {
                const localData = localStorage.getItem('usuariosDB');
                bancoGlobal = localData ? JSON.parse(localData) : {};
            }
        } catch (e) { console.error("Erro banco:", e); }

        const listaBloqueados = JSON.parse(localStorage.getItem('listaBloqueados')) || [];
        const matchesExistentes = JSON.parse(localStorage.getItem('meus_matches')) || [];
        const vistos = JSON.parse(localStorage.getItem('perfis_vistos') || '[]');

        const dadosLogado = bancoGlobal[usuarioLogadoNome] || {};
        const meuInteresse = (dadosLogado.interesse || "Todos").toLowerCase();

        let listaFormatada = Object.keys(bancoGlobal)
        .filter(nome => {
            const nomeBanco = nome.toLowerCase().trim();
            const nomeLogado = usuarioLogadoNome.toLowerCase().trim();
            const u = bancoGlobal[nome];

            if (!u) return false;
            if (nomeBanco === nomeLogado) return false;
            if (!u.foto && (!u.fotos || u.fotos.length === 0)) return false;
            if (vistos.includes(nome) || vistos.includes(`${nome}, ${u.idade}`)) return false;

            const isBloqueado = listaBloqueados.some(b => 
b.nome.toLowerCase().split(',')[0].trim() === nomeBanco
);
            const isMatch = matchesExistentes.some(m => m.toLowerCase().trim() === nomeBanco);

            if (isBloqueado || isMatch) return false;

            const generoDela = (u.genero || "Mulher").toLowerCase();
            if (meuInteresse === "homens") return generoDela === "homem";
            if (meuInteresse === "mulheres") return generoDela === "mulher";

            return true;
        })
        .map(nome => {
            const u = bancoGlobal[nome];
            let fotosReais = (u.fotos && u.fotos.length > 0)
? u.fotos
: [u.foto || "https://i.imgur.com/6VBx3io.png"];
            return {
                name: `${nome}, ${u.idade || '22'}`,
                imgs: fotosReais,
                dist: "Perto de você",
                job: u.profissao || "Membro Connect",
                bio: u.bio || "Olá!",
                interests: u.interesses || ["Amizade"],
                tag: "Novo"
            };
        })
    ...
    let profiles = getProfilesFromDB();
    let currentIdx = 0;
    ...
    function updateProfileUI() {
        ...
        document.getElementById('profile-img').src = p.imgs[currentPhotoIdx] || p.imgs[0];
    ...
    function openProfileSheet() {
        const p = profiles[currentIdx];
        if(!p) return;
        const isGold = localStorage.getItem('usuario_gold') === 'true';
        const overlay = document.getElementById('premium-overlay');
        const conteudo = document.getElementById('conteudo-perfil');
        if (!isGold) { overlay.classList.remove('hidden'); conteudo.classList.add('premium-blur'); }
        else { overlay.classList.add('hidden'); conteudo.classList.remove('premium-blur'); }
        document.getElementById('sheet-nome').innerText = p.name;
    ...
    function closeProfileSheet() { document.getElementById('modal-perfil-explorar').classList.add('hidden'); }
    function toggleMenuOpcoes() { document.getElementById('dropdown-opcoes').classList.toggle('hidden'); }

    function confirmarBloqueio() {
        const p = profiles[currentIdx];
        let bloqueados = JSON.parse(localStorage.getItem('listaBloqueados')) || [];
        bloqueados.push({ nome: p.name, foto: p.imgs[0], data: new Date().toLocaleDateString('pt-BR') });
        localStorage.setItem('listaBloqueados', JSON.stringify(bloqueados));
        closeProfileSheet();
        nextProfile('dislike');
    }

    function denunciarUsuario() {
        exibirToast("Denúncia Enviada", "error");
        setTimeout(() => confirmarBloqueio(), 1500);
    }

    function nextProfile(action) {
        if (!profiles || !profiles[currentIdx]) return;
        const p = profiles[currentIdx]; 

        let vistos = JSON.parse(localStorage.getItem('perfis_vistos') || '[]');
        vistos.push(p.name);
        localStorage.setItem('perfis_vistos', JSON.stringify(vistos));

        if (action === 'like') {
            let curtidos = JSON.parse(localStorage.getItem('perfis_curtidos') || '[]');
            curtidos.push(p.name);
            localStorage.setItem('perfis_curtidos', JSON.stringify(curtidos));
            
            const nomeSolo = p.name.split(',')[0].trim(); 
            const conexoes = JSON.parse(localStorage.getItem('contatos_ativos') || '{}');
            conexoes[nomeSolo] = p.imgs[0];
            localStorage.setItem('contatos_ativos', JSON.stringify(conexoes));
            
            let matchesGeral = JSON.parse(localStorage.getItem('meus_matches') || '[]');
            if (!matchesGeral.includes(nomeSolo)) {
                matchesGeral.push(nomeSolo);
                localStorage.setItem('meus_matches', JSON.stringify(matchesGeral));
            }
            if(window.parent && window.parent.triggerMatch) window.parent.triggerMatch();
        }

        card.style.transform = action === 'dislike' ? 'translateX(-150%) rotate(-20deg)' : 'translateX(150%) rotate(20deg)';
    ...
    window.onload = () => { try { updateProfileUI(); } catch (e) { console.error(e); } };
    ...
    const handleTouchEnd = (e) => {
        ...
        if (Math.abs(diffX) > window.innerWidth * 0.25) { 
            nextProfile(diffX > 0 ? 'like' : 'dislike'); 
        } else { 
